...
/* Standard Algorithms to be used in Void Solve*/
...
bool isPalindrome(vector<int> &vec,int n){
  bool flag=false;
  for(int i=0;i<n/2;i++){
    if(vec[i]!=vec[n-1-i]){
      flag=true;
...
//  Code starts here
// first you have to create three files 
// error.txt input.txt and output.txt
void solve(){
  int n;cin>>n;
  // string s;cin>>s;
  // vector<int> v(n);
  ...
  int c = n, c0 = 0, c1 = 0;
    ...
    if (c0 + c1 == 31 || c0 + c1 == 0) cout<< -1; return;
